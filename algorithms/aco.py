...
def antProduct(source, destination, matrix, environment):
    if matrix[source][destination] == 0:
        return 1
    a = pow(environment.pheromoneMatrix[source][destination], environment.alpha)
    b = pow((1.0 / matrix[source][destination]), environment.beta)
    return a * b
...
def simulateAnts(citiesNo, matrix, environment):
    ...
    for ant in range(0, environment.colonySize):
        ...
        # print("aa")
        # print(environment.ants[ant].pathIndex, citiesNo)
        if (environment.ants[ant].pathIndex < citiesNo):
            ...
            environment.ants[ant].pathIndex = environment.ants[ant].pathIndex + 1
            environment.ants[ant].pathTaken[environment.ants[ant].pathIndex] = environment.ants[ant].nextCity
...
if __name__ == '__main__':
    ...
    for file in inputFiles:
        ...
        environment = Environment(citiesNo)
        initializeSimulation(environment, citiesNo)

        currentIteration = 0
        while currentIteration < environment.maxIterations:
            currentIteration += 1
            if simulateAnts(citiesNo, matrix, environment) == 0:
                updateTrails(citiesNo)
                if currentIteration != environment.maxIterations:
                    restartAnts(environment, citiesNo)
        print(environment.bestTourLength)
